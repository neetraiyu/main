                        ...
                        <tbody>
                            @forelse($tasks as $task)
                                <tr id="task-{{ $task->id }}">
                                    ...
                                    @php
                                    preg_match('/\d+/', $task->message, $matches);
                                    $duration = isset($matches[0]) ? (int) $matches[0] : 0;
                                    @endphp
                                    <td>{{ $task->accepted_at ? number_format($duration - $task->accepted_at->diffInDays(now()), 0) : number_format($duration, 0) }}</td>
                                    ...
                                    <td>
                                        @if ($task->status == 'pending')
                                            ...
                                            @elseif ($task->status == 'completed')
                                                <button class="btn btn-outline-primary view-btn" data-id="{{ $task->id }}">View</button>                                        @endif
                                    </td>
                                </tr>
                            @empty
                                <tr>
        ...
        function updateTaskStatus(taskId, action) {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#task-' + taskId + ' td:nth-child(7)').text(response.status.charAt(0).toUpperCase() + response.status.slice(1));
                    if (response.status === 'processing') {
                        $('#task-' + taskId + ' td:nth-child(8)').html('<button class="btn btn-primary btn-sm" onclick="updateTaskStatus(' + taskId + ', \'complete\')">Complete</button>');
                    }else if(response.status === 'completed'){
                        $('#task-' + taskId + ' td:nth-child(8)').html('<td class="text-success">completed</td>');
                    }
